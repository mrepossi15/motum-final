...
<div class="max-w-4xl mx-auto p-4 mt-6 relative">
    <a href="{{ route('trainer.profile') }}" class="text-orange-500 font-medium">&lt; Volver a mi perfil</a>
    <div class="bg-white pt-6 rounded-lg mt-4 shadow-md p-6">
        ...
        <form action="{{ route('trainer.update') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 sm:gap-6 items-start">
                <!-- Foto de Perfil -->
                <div class="col-span-1 flex flex-col items-center md:items-start" x-data="photoPreview('{{ $trainer->profile_pic ? asset('storage/' . $trainer->profile_pic) : null }}')">
                ...
                <div class="col-span-3 space-y-4">
                    ...
                    <x-form.input name="name" label="Nombre" value="{{ $trainer->name }}" required />
                    <x-form.input name="email" label="Correo Electrónico" type="email" value="{{ $trainer->email }}" required />
                    <x-form.input name="phone" label="Teléfono" type="tel" value="{{ $trainer->phone }}" />
                </div>
            </div>
            ...
            <div class="hidden md:block"></div>
                <div class="md:col-span-3">
                    <div class="space-y-4">
                    <x-form.textarea name="biography" label="Biografía" value="{{ $trainer->biography }}" rows="4" />
                    <x-form.input name="certification" label="Certificación Profesional" value="{{ $trainer->certification }}" rows="4" />
                    </div>
                    <div class="md:col-span-2 mt-4 grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            <label for="certification_pic" class="block text-gray-700 font-bold">Certificación</label>
                            <input type="file" id="certification_pic" name="certification_pic" accept="image/*"
